...
public class SrtPlayService
{
	...
	{
		if (BasicFileUtil.writeFileString(favoriteTxt, getFavoriteCurrContent(), "UTF-8", true))
		{
	...
	{
		String filePath = thumbPicFolder + getCurFile().replace(srtActivity.srtFolder, "");
		int i = filePath.lastIndexOf(".");
		filePath = filePath.substring(0, i);
		filePath = filePath + File.separator + TextFormatUtil.getFileNameNoExtend(getCurFile()) + "_p1.pic";
		return filePath;
	}
	...
	{
		return BasicDateUtil.getCurrentDateTimeString() + " \"" + getCurFile().replace(srtActivity.srtFolder, "") + "\" " + getCurrentPlaySrtInfo() + "\r\n";
	}
	...
	public String[] getDirs()
	{
		...
		{
			leftArr[i++] = BasicStringUtil.subString(folder.getName(), 0, FOLDER_NAME_MAXLEN);
		}
		return leftArr;
	}
