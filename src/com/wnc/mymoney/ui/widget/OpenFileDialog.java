...
import com.wnc.mymoney.ui.helper.CallbackBundle;

public class OpenFileDialog
{
    public static String tag = "OpenFileDialog";
    static final public String sRoot = "/storage/emulated/0/";
    ...
    static class FileSelectView extends ListView implements OnItemClickListener
    {
        ...
        private int refreshFileList()
        {
            ...
            if (files == null)
            {
                Log.e("opendialog", "refreshFileLis...访问出错! ");
                // 访问出错
            ...
            {
                this.list = new ArrayList<Map<String, Object>>(files.length);
            }
            ...
            for (File file : files)
            {
                ...
                else if (file.isFile())
                {
                    ...
                    if (this.suffix == null
                            ...
                            || (sf.length() > 0 && this.suffix.indexOf("." + sf
                                    + ";") >= 0))
                    {
                        Map<String, Object> map = new HashMap<String, Object>();
                        map.put("name", file.getName());
            ...
            this.list.addAll(lfolders); // 先添加文件夹，确保文件夹显示在上面
            this.list.addAll(lfiles); // 再添加文件

            SimpleAdapter adapter = new SimpleAdapter(getContext(), this.list,
                    R.layout.filedialogitem, new String[]
                    { "img", "name", "path" }, new int[]
