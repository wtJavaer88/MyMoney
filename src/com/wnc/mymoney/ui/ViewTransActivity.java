...
import com.wnc.mymoney.util.FestivalUtil;
        ...
        HorGestureDetectorListener, OnClickListener
{
    ...
    public void onClick(View v)
    {
        if (v.getId() == R.id.trade_time_row_btn)
        {
            if (viewedTrade != null)
            {
                String spendDay = viewedTrade.getCreatetime().substring(0,
                        viewedTrade.getCreatetime().indexOf(" "));
                String fesStr = FestivalUtil.getFesStr(spendDay);
                ToastUtil.showLongToast(this,
                        fesStr
                                + " "
                                + BasicDateUtil.getGBWeekString(spendDay)
                                        .replace("七", "天"));
            }
        }
    }
