...
public class ViewTransActivity extends BaseActivity implements HorGestureDetectorListener, OnClickListener
{
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		initData();
		this.gestureDetector = new GestureDetector(this, new MyGestureDetector(0.2, 0, this));
		enter = true;
		initViews();
	}
	...
	{
		this.tradeTimeBT.setText(BasicDateUtil.getDateTimeString(TextFormatUtil.getFormatedDate(viewedTrade.getCreatetime())));
		reSetCategoryTV(CostTypeUtil.getCostTypeName(viewedTrade.getCostlevel_id()) + "-->" + CostTypeUtil.getCostTypeName(viewedTrade.getCostdesc_id()));
	...
	private void asynAppend(final String segment)
	{
		new Thread(new Runnable()
		{
			...
			public void run()
			{
				...
				if (isVoiceFile)
				{
					path = MyAppParams.getInstance().getTmpVoicePath() + segment.trim();
				}
				else if (isPicFile)
				{
					path = MyAppParams.getInstance().getTmpPicPath() + segment.trim();
				}
				else if (isVideoFile)
				{
					path = MyAppParams.getInstance().getTmpVideoPath() + segment.trim();
				}
				...
				if (file != null && file.isFile() && file.exists())
				{
					if (isVoiceFile)
					{
						ClickableVoiceRichText clickableVoiceRichText = new ClickableVoiceRichText(ViewTransActivity.this, path);
						msg.obj = clickableVoiceRichText.getSequence();
					}
					else if (isPicFile)
					{
						ClickablePicRichText clickablePicRichText = new ClickablePicRichText(ViewTransActivity.this, path);
						msg.obj = clickablePicRichText.getSequence();
						picTexts.add(clickablePicRichText);
					}
					else if (isVideoFile)
					{
						ClickableVideoRichText clickableVideoRichText = new ClickableVideoRichText(ViewTransActivity.this, path);
	...
	public void getNearTrade(String flag)
	{
		...
		if (flag.equals("L"))
		{
			nearTrade = TransactionsDao.getPreTrade(viewedTrade.getCreatetime());
		}
		else if (flag.equals("R"))
		{
			nearTrade = TransactionsDao.getNextTrade(viewedTrade.getCreatetime());
		}
	...
	public void onClick(View v)
	{
		if (v.getId() == R.id.trade_time_row_btn)
		{
			if (viewedTrade != null)
			{
				ToastUtil.showLongToast(this, BasicDateUtil.getGBWeekString(viewedTrade.getCreatetime().substring(0, viewedTrade.getCreatetime().indexOf(" "))).replace("七", "天"));
			}
		}
	}
}
