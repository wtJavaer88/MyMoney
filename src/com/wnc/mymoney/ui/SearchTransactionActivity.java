...
public class SearchTransactionActivity extends DataViewActivity implements View.OnClickListener
{
	...
	private final String[] members = MemberDao.getAllMembersForSearch().toArray(new String[MemberDao.getCounts() + 1]);
	private final String[] picItems = new String[] { "全部备注", "无图", "有图" };
	...
	private void initDateRange()
	{
		...
		this.dayRange = (DayRangePoint) localIntent.getSerializableExtra("dayrange");
	...
	private void initViews()
	{
			...
			public void afterTextChanged(Editable s)
			{
				...
				SearchTransactionActivity.this.mHandler.post(new Runnable()
				{
					...
					public void run()
					{
						if (word.length() > 0)
						{
							SearchTransactionActivity.this.search_clear_btn.setVisibility(View.VISIBLE);
						}
						else
						{
							SearchTransactionActivity.this.search_clear_btn.setVisibility(View.INVISIBLE);
						}
		...
		group.setOnCheckedChangeListener(new OnCheckedChangeListener()
		{
			...
			public void onCheckedChanged(RadioGroup arg0, int arg1)
			{
				...
				RadioButton rb = (RadioButton) SearchTransactionActivity.this.findViewById(radioButtonId);
				// 更新文本内容，以符合选中项
	...
	private void showCostTypeWheel()
	{
		WheelDialogShowUtil.showRelativeDialog(this, "选择分类", leftData, rightData, selectedLeftIndex, selectedRightIndex, new AfterWheelChooseListener()
		{
			...
			public void afterWheelChoose(Object... objs)
			{
				...
				MyWheelBean rbean = rightData.get(lbean).get(selectedRightIndex);
				if (rbean.getId() != NONE_TYPE_ID)
				{
					typeSelBtn.setText(lbean.getName() + "-->" + rbean.getName());
				}
				else
				{
					typeSelBtn.setText(lbean.getName());
				}
				selectedPath = "/" + lbean.getId() + "/" + rbean.getId();
	...
	private void createNewAdapter(String keyword)
	{
		...
		this.adapter = new SimpleAdapter(this, mData, R.layout.nav_year_trans_lv_item, new String[] { "icon", "name", "photo", "memo", "cost" }, new int[] { R.id.item_icon_iv, R.id.item_name_tv, R.id.photo_flag_iv, R.id.memo_tv, R.id.cost_tv });
		this.search_expense_lv.setAdapter(this.adapter);
		TransItemClickListener transItemClickListener = new TransItemClickListener(this, this.search_expense_lv);
	...
	private void getSearchMapData(String keyword)
	{
		...
		List<Trade> tradeItems = TransactionsDao.getSearchTransactionWithDateRange(haspic, member, selectedPath, keyword, this.dayRange.getStartDay(), this.dayRange.getEndDay());
	...
	private void dataReOrder()
	{
		Collections.sort(mData, new Comparator<Map<String, Object>>()
		{
			...
			public int compare(Map<String, Object> o1, Map<String, Object> o2)
			{
				...
				{
					String com1 = o1.get("costlevelid") + "" + o1.get("costdescid") + "";
					String com2 = o2.get("costlevelid") + "" + o2.get("costdescid") + "";
					return com1.compareTo(com2);
				}
				return 0;
			}
