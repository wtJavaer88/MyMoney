...
public class AddOrEditTransActivity extends BaseActivity implements OnClickListener
{
	...
	private final String[] members = MemberDao.getAllMembers().toArray(new String[MemberDao.getCounts()]);
	...
	private void initCombobox()
	{
		...
		this.m_combobox.setListViewOnClickListener(new ListViewItemClickListener()
		{
			...
			public void onItemClick(int position)
			{
				...
				{
				case 0:
					reSetCategoryTV(defaultOutLevelName + "-->" + defaultOutDescTypeName);
					curTradeType = CategoryDao.defaultOutTradeType;
					curDescTradeType = CategoryDao.defaultOutDescTradeType;
					break;
				case 1:
					reSetCategoryTV(defaultInLevelName + "-->" + defaultInDescTypeName);
	...
	private void setViewsInCurrModel()
	{
		if (this.isAdd)
		{
			...
			reSetCategoryTV(this.defaultOutLevelName + "-->" + this.defaultOutDescTypeName);
		...
		{
			((TextView) findViewById(R.id.titlebar_right_btn)).setVisibility(View.INVISIBLE);
			findViewById(R.id.add_trans_save_and_new_btn).setVisibility(View.GONE);
	...
	private void share()
	{
		...
		share.putExtra(Intent.EXTRA_STREAM, Uri.fromFile(new File(MyAppParams.getInstance().getTmpPicPath() + "332680175.jpg")));
		...
		builder.setItems(this.picMenu, new DialogInterface.OnClickListener()
		{
			...
			public void onClick(DialogInterface arg0, int arg1)
			{
				if (arg1 == 0)
				{
					...
					{
						mPhotoFile = new File(savePicDir, BasicDateUtil.getCurrentDateTimeString() + ".jpg");
						mPhotoFile.delete();
				...
				{
					Intent intent = new Intent(AddOrEditTransActivity.this, MovieActivity.class);
	...
	{
		WheelDialogShowUtil.showCurrDateTimeDialog(this, this.tradeTimeBT.getText().toString(), new AfterWheelChooseListener()
		{
			...
			public void afterWheelChoose(Object... objs)
			{
				...
				{
					ToastUtil.showLongToast(getApplicationContext(), "请检查是否有" + TextFormatUtil.addChnToDay(day) + "这一天!");
				}
				else
				{
	...
	private void showCostTypeWheel()
	{
		...
		WheelDialogShowUtil.showRelativeDialog(this, "选择分类", leftData, rightData, selectedLeftIndex, selectedRightIndex, new AfterWheelChooseListener()
		{
			@Override
			public void afterWheelChoose(Object... objs)
			{
				setCostIndex(Integer.valueOf(objs[0].toString()), Integer.valueOf(objs[1].toString()));
			}
		});
	...
	{
		Intent localIntent = new Intent(AddOrEditTransActivity.this, CostKeyboardActivity.class);
	...
	{
		return JSONArray.parseObject(ClipBoardUtil.getClipBoardContent(this), Trade.class);
	}
	...
	private void backupFilesInTrade(Trade trade)
	{
		for (String segment : TextFormatUtil.getSegmentsInMemo(trade.getMemo()))
		{
			...
			{
				BackupFilesHolder.addBackupFile(MyAppParams.getInstance().getTmpPicPath() + segment);
			}
			else if (FileTypeUtil.isVoiceFile(segment.trim().toLowerCase()))
			{
				BackupFilesHolder.addBackupFile(MyAppParams.getInstance().getTmpVoicePath() + segment);
			}
			else if (FileTypeUtil.isVideoFile(segment.trim().toLowerCase()))
			{
				BackupFilesHolder.addBackupFile(MyAppParams.getInstance().getTmpVideoPath() + segment);
			}
		}
	}
	...
	private void setViewsByExistTrade(Trade existTrade)
	{
		...
		this.tradeTimeBT.setText(BasicDateUtil.getDateTimeString(TextFormatUtil.getFormatedDate(existTrade.getCreatetime())));
		reSetCategoryTV(CostTypeUtil.getCostTypeName(existTrade.getCostlevel_id()) + "-->" + CostTypeUtil.getCostTypeName(existTrade.getCostdesc_id()));
		...
		for (int i = 0; i < outTradeTypes.size(); i++)
		{
			...
			{
				selectedLeftIndex = i;
				List<? extends MyWheelBean> list = outTradeDescTypes.get(outTradeTypes.get(i));
	...
	{
		parseTrade.setCreatelongtime(TextFormatUtil.getFormatedDate(parseTrade.getCreatetime()).getTime());
	...
	private Trade getTradeFromViews()
	{
		...
		trade.setModifytime(TextFormatUtil.getDateStrForDb(BasicDateUtil.getCurrentDateTimeString()));
		trade.setCreatelongtime(TextFormatUtil.getFormatedDate(timeStr).getTime());
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data)
	{
		...
		if (requestCode == this.CAMERA_RESULT && resultCode == RESULT_OK)
		{
			...
			{
				System.out.println("mPhotoFile::" + this.mPhotoFile.getAbsolutePath());
		...
		if (requestCode == this.LOAD_IMAGE_RESULT && resultCode == RESULT_OK && null != data)
		{
		...
		{
			String cost = TextFormatUtil.getFormatMoneyStr(data.getDoubleExtra("cost", 0d));
			this.tranCostBT.setText(cost);

			String ids = getAutoSelectedIndexByCost(BasicNumberUtil.getDouble(cost));
		...
		if (requestCode == this.VOICE_RESULT && data != null)
		{
			try
			{
				...
				if (amrFile != null)
				{
					System.out.println("audio file:" + amrFile.getAbsolutePath());
					backupAndInsertMemo(saveVoiceDir, amrFile.getAbsolutePath(), ".amr");
				}
			}
			catch (Exception e)
			{
	 ...
	 * @param moviePath
	 */
	private void backupAndInsertMemo(String saveDir, String memoFilePath, String suffixType)
	{
	...
	private void insertFileToMemo(String imgPath)
	{
		...
		try
		{
			...
			// 设置光标的位置保持不变
			Selection.setSelection(this.memoET.getText(), Math.min(index + imgPath.length(), sb.length() + imgPath.length()));
		}
		catch (Exception ex)
		{
