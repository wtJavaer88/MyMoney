		...
		View.OnClickListener, HorGestureDetectorListener
{
	...
	public MyExpandableListAdapter yearAdapter;
	...
	public void initData()
	{
		...
		if (this.mData.size() > 0)
		{
			...
			{
				yearAdapter = new MyExpandableListAdapter(this, dayRange);
				((ExpandableListView) this.expanseLV).setAdapter(yearAdapter);
			}
			else
			{
	...
	public void onClick(View v)
	{
		...
		else if (v == this.importBt)
		{
			List<Trade> copyList = null;
			if (adapter == null)
			{
				copyList = yearAdapter.getTradeItemsInDays();
			}
			else
			{
				copyList = this.adapter.getTradeItemsInDays();
			}
			ClipBoardUtil.setTradeContent(this, JSON.toJSONString(copyList));
	...
	public void doLeft()
	{
		...
		{
			((QExListView) expanseLV).clearTipTv();
		}
	...
	private void reComputeTotalData(ListView arg0, int arg2)
	{
				...
				.entrySet())
		{
			...
			{
				Map<String, Object> map = (HashMap) arg0
						.getItemAtPosition(arg2);
				Map<String, Object> totalDayMap = mData.get(item.getKey());
				double in = BasicNumberUtil.getDouble(this.inboundTV.getText()
						.toString().trim());
