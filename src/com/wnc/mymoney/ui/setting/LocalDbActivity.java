...
public class LocalDbActivity extends Activity
{
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		listView.setOnItemLongClickListener(new OnItemLongClickListener()
		{
			...
			public boolean onItemLongClick(AdapterView<?> arg0, View arg1, final int arg2, long arg3)
			{
				...
				final String[] menuItems = new String[] { "还原", "删除", "分享" };
				new AlertDialog.Builder(LocalDbActivity.this).setTitle("对记录进行操作").setItems(menuItems, new DialogInterface.OnClickListener()
				{
					@Override
					public void onClick(DialogInterface dialog, int which)
					{
						final String dbpath = String.valueOf(map.get("path"));
						try
						{
							if (which == 0)
							{
								...
								{
									BackUpDataUtil.backupDatabase(activity);
								}
								if (MoveDbUtil.moveSdCardDb(dbpath, activity.getDatabasePath("money.db")))
								{
									ToastUtil.showLongToast(getApplicationContext(), "还原数据成功!");
							...
							{
								ConfirmUtil.confirmDelete(activity, "确定要删除数据吗?", new PositiveEvent()
								{
									@Override
									public void onPositive()
									{
										boolean b = BasicFileUtil.deleteFile(dbpath);
										if (b)
										{
											ToastUtil.showLongToast(getApplicationContext(), "删除数据文件成功!");
											listView.setAdapter(getAdapter());
										}
										else
										{
											ToastUtil.showLongToast(getApplicationContext(), "删除数据文件失败!");
										}
									}
								});
							...
							{
								ShareUtil.shareFile(activity, dbpath);
							}
						}
						catch (Exception ex)
						{
							ToastUtil.showShortToast(LocalDbActivity.this, ex.getMessage());
						}
						finally
						{
						}
	...
	private ListAdapter getAdapter()
	{
		...
		for (int i = 0; i < files.length; i++)
		{
			...
			map.put("name", TextFormatUtil.getFileNameNoExtend(files[i].getName()));
			map.put("path", files[i].getAbsolutePath());
			list.add(map);
		}

		return new SimpleAdapter(this, list, R.layout.localdb_item, new String[] { "name" }, new int[] { R.id.dbname });
	}
}