...
public class CategoryDao
{
	...
	private static List<MyWheelBean> getsonList(Category category)
	{
		...
		for (Category category_son : allList)
		{
			if (category_son.getParent_id().equals(category.getId() + ""))
			{
				...
				if (defaultOutDescTradeType == Integer.MIN_VALUE && category_son.getPath().startsWith("/-1/"))
				{
					defaultOutDescTradeType = category_son.getId();
				}
				if (defaultInDescTradeType == Integer.MIN_VALUE && category_son.getPath().startsWith("/-2/"))
				{
	...
	private static void getTypesFromdb()
	{
		...
		Cursor c = db.rawQuery("SELECT * FROM category WHERE DEPTH>0 ORDER BY ORDERED ASC", new String[] {});
		while (c.moveToNext())
		{
