...
public class DragListView extends ListView
{
    ...
    public boolean onTouchEvent(MotionEvent ev)
    {
                ...
                && !isLock)
        {
            ...
            case MotionEvent.ACTION_UP:
                ...
                onDrop(upY);
                // if(false == isMoving){
                // mHandler.sendMessage(mHandler.obtainMessage(MSG_DRAG_STOP,
                // upY, 0));
                // }else{
                // mHandler.sendMessageDelayed(mHandler.obtainMessage(MSG_DRAG_STOP,
                // upY, 0), ANIMATION_DURATION);
                // }
                break;
            case MotionEvent.ACTION_MOVE:
                int moveY = (int) ev.getY();
                // mHandler.sendMessage(mHandler.obtainMessage(MSG_DRAG_MOVE,
                // moveY, 0));
                onDrag(moveY);
                // test(moveY);
                testAnimation(moveY);
    ...
    // return;
    // }
    // if(tempPosition != AdapterView.INVALID_POSITION && tempPosition !=
    // dragPosition){
    ...
    private void testAnimation(int y)
    {
        ...
        for (int i = 1; i <= count; i++)
        {
            ...
            // ViewGroup invisibleView = (ViewGroup)getChildAt(startPosition -
            // getFirstVisiblePosition());
            // Animation animation2;
            // int fromY = (tempPosition - startPosition -1) *
            // mItemVerticalSpacing;
            // int toY = tempPosition * mItemVerticalSpacing;
            // animation2 = getAnimation(fromY, toY);
            //
            // invisibleView.startAnimation(animation2);
            // ViewGroup invisibleView = (ViewGroup)getChildAt(tempPosition -
            // getFirstVisiblePosition());
