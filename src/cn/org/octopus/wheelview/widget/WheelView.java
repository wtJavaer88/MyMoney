...
public class WheelView extends View
{
    ...
    /** 顶部和底部的阴影颜色 */
    // private static final int[] SHADOWS_COLORS = new int[] { 0xFF5436EE,
    // 0x0012CEAE, 0x0012CEAE };
    ...
    private void initResourcesIfNecessary()
    {
        ...
        {
            this.itemsPaint = new TextPaint(Paint.ANTI_ALIAS_FLAG
        ...
        {
            this.valuePaint = new TextPaint(Paint.ANTI_ALIAS_FLAG
                    | Paint.FAKE_BOLD_TEXT_FLAG | Paint.DITHER_FLAG);
            this.valuePaint.setTextSize(this.TEXT_SIZE);
            this.valuePaint.setShadowLayer(0.1f, 0, 0.1f, 0xFFC0C0C0);
        }

        // 选中的条目背景
        if (this.centerDrawable == null)
        {
            this.centerDrawable = getContext().getResources().getDrawable(
    ...
    private int getDesiredHeight(Layout layout)
    {
         ...
         */
        int desired = getItemHeight() * this.visibleItems - this.ITEM_OFFSET
                * 2 - ADDITIONAL_ITEM_HEIGHT;
    ...
    private String buildText(boolean useCurrentValue)
    {
         ...
         */
        for (int i = this.currentItem - addItems; i <= this.currentItem
                + addItems; i++)
        {
            // 如果在滚动
    ...
    private int getMaxTextLength()
    {
         ...
         */
        for (int i = Math.max(this.currentItem - addItems, 0); i < Math.min(
                this.currentItem + this.visibleItems, adapter.getItemsCount()); i++)
        {
    ...
    private int getItemHeight()
    {
        ...
        if (this.itemHeight != 0)
        {
            ...
            // 如果条目的高度为 0, 并且普通条目布局不为null, 条目个数大于 2
                ...
                && this.itemsLayout.getLineCount() > 2)
        {
            /*
             * itemsLayout.getLineTop(2) : 获取顶部第二行上面的垂直(y轴)位置, 如果行数等于
             */
            this.itemHeight = this.itemsLayout.getLineTop(2)
                    - this.itemsLayout.getLineTop(1);
            return this.itemHeight;
        }
    ...
    private int calculateLayoutWidth(int widthSize, int mode)
    {
        ...
        // 获取最长的条目显示字符串字符个数,这儿做了改变,可能是中文字符的时候需要翻倍
        int maxLength = getMaxTextLength() * 2;
        ...
        // 总宽度加上一些间距
        this.itemsWidth += ADDITIONAL_ITEMS_SPACE; // make it some more

        // 计算 label 的长度
        this.labelWidth = 0;
        if (this.label != null && this.label.length() > 0)
        {
            this.labelWidth = (int) Math.ceil(Layout.getDesiredWidth(
                    this.label, this.valuePaint));
        ...
        {
            // 未定义模式
            width = this.itemsWidth + this.labelWidth + 2 * PADDING;
        ...
        if (recalculate)
        {
            ...
            if (this.labelWidth > 0)
            {
                double newWidthItems = (double) this.itemsWidth * pureWidth
            ...
            {
                this.itemsWidth = pureWidth + LABEL_OFFSET; // no label
            }
        }
    ...
    private void createLayouts(int widthItems, int widthLabel)
    {
         ...
         */
        // System.out.println("我 要 改 变 widthItems: " + widthItems + " text:"
        // + this.adapter.getItem(0));
        // widthItems = MyAppParams.getScreenWidth() / 2;
        if (this.itemsLayout == null
                || this.itemsLayout.getWidth() > widthItems)
        {
            /*
             ...
             */
            this.itemsLayout = new StaticLayout(
                    buildText(this.isScrollingPerformed), this.itemsPaint,
                    widthItems,
        ...
        if (!this.isScrollingPerformed
                && (this.valueLayout == null || this.valueLayout.getWidth() > widthItems))
        {
            String text = getAdapter() != null ? getAdapter().getItem(
                    this.currentItem) : null;
            this.valueLayout = new StaticLayout(text != null ? text : "",
                    this.valuePaint, widthItems,
                    widthLabel > 0 ? Layout.Alignment.ALIGN_OPPOSITE
                            : Layout.Alignment.ALIGN_CENTER, 1,
                    ADDITIONAL_ITEM_HEIGHT, false);
        ...
        if (widthLabel > 0)
        {
            if (this.labelLayout == null
                    || this.labelLayout.getWidth() > widthLabel)
            {
                this.labelLayout = new StaticLayout(this.label,
                        this.valuePaint, widthLabel,
    ...
    protected void onDraw(Canvas canvas)
    {
        ...
        if (this.itemsWidth > 0)
        {
            ...
            canvas.translate(PADDING, -this.ITEM_OFFSET);
            // 绘制普通条目
            drawItems(canvas);
            // 绘制选中条目
            drawValue(canvas);
    ...
    private void drawShadows(Canvas canvas)
    {
        ...
        this.bottomShadow.setBounds(0, getHeight() - getHeight()
                / this.visibleItems, getWidth(), getHeight());
    ...
    private void drawValue(Canvas canvas)
    {
        ...
        {
            canvas.save();
            canvas.translate(this.itemsLayout.getWidth() + LABEL_OFFSET,
                    bounds.top);
        ...
        {
            canvas.save();
            canvas.translate(0, bounds.top + this.scrollingOffset);
    ...
    private void drawItems(Canvas canvas)
    {
        ...
        canvas.translate(0, -top + this.scrollingOffset);
    ...
    private void drawCenterRect(Canvas canvas)
    {
        ...
        this.centerDrawable.setBounds(0, center - offset, getWidth(), center
                + offset);
    ...
    private void doScroll(int delta)
    {
        ...
        // 将滚动后剩余的小数部分保存
        this.scrollingOffset = offset - count * getItemHeight();
        if (this.scrollingOffset > getHeight())
        {
            this.scrollingOffset = this.scrollingOffset % getHeight()
                    + getHeight();
        }
    }
    ...
    private SimpleOnGestureListener gestureListener = new SimpleOnGestureListener()
    {
                ...
                float velocityY)
        {
            ...
            WheelView.this.lastScrollY = WheelView.this.currentItem
                    * getItemHeight() + WheelView.this.scrollingOffset;
            // 如果可以循环最大值是无限大, 不能循环就是条目数的高度值
            int maxY = WheelView.this.isCyclic ? 0x7FFFFFFF
                    : WheelView.this.adapter.getItemsCount() * getItemHeight();
             ...
             * maxY : y 轴滚动的最大值
             */
            WheelView.this.scroller.fling(0, WheelView.this.lastScrollY, 0,
                    (int) -velocityY / 2, 0, 0, minY, maxY);
            setNextMessage(WheelView.this.MESSAGE_SCROLL);
            return true;
        }
    };
    ...
    private Handler animationHandler = new Handler()
    {
        ...
        public void handleMessage(Message msg)
        {
             ...
             */
            if (Math.abs(currY - WheelView.this.scroller.getFinalY()) < MIN_DELTA_FOR_SCROLLING)
            {
            ...
            else if (msg.what == WheelView.this.MESSAGE_SCROLL)
            {
                justify();
            }
            else
    ...
    private void justify()
    {
         ...
         */
        boolean needToIncrease = offset > 0 ? this.currentItem < this.adapter
                .getItemsCount() : this.currentItem > 0;
        if ((this.isCyclic || needToIncrease)
    ...
    public void scroll(int itemsToScroll, int time)
    {
         ...
         */
        this.scroller.startScroll(0, this.lastScrollY, 0, offset
                - this.lastScrollY, time);
